<?php
	...
	while($row=mysqli_fetch_assoc($sql))
	{
		if($f)
			...
			$f=true;
		echo '{"name":'.escape($row["name"]." ".$row["surname"]).',"id":'.$row["rid"].',"id2":'.intval($row["id2"]).',"zadanie":"'.$row["zadanienazwa"].'","status":'.$row["status"].',"wynik":'.$row["wynik"].'}';
	}
	echo ']}';
?>
